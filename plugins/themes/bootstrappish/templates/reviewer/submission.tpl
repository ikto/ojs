...
<div id="submissionToBeReviewed" class="col-md-12 mag-innert-left">
...
<div id="reviewSchedule" class="col-md-12 mag-innert-left">
...
<div id="reviewSteps" class="col-md-12 mag-innert-left">
			...
			<tr valign="top">
				...
				<td>
					<table width="100%" class="table table-striped">
						...
						{else}
							<tr><td class="nodata"><p class="help-block">{translate key="reviewer.article.restrictedFileAccess"}</p></td></tr>
						{/if}
					</table>
				</td>
			</tr>
			...
			{if $currentJournal->getSetting('requireReviewerCompetingInterests')}
				<tr valign="top">
					...
					<td>
						...
						{else}
							<form role="form" action="{url op="saveCompetingInterests" reviewId=$reviewId}" method="post">
								<div class="form-group"><textarea {if $cannotChangeCI}disabled="disabled" {/if}name="competingInterests" class="form-control" id="competingInterests" rows="5" cols="40">{$reviewAssignment->getCompetingInterests()|escape}</textarea></div><br />
					...
					<table class="table table-striped" width="100%">
						...
						{foreachelse}
							<tr valign="top">
								...
								<td class="nodata">
									<p class="help-block">{translate key="common.none"}</p>
								</td>
							</tr>
						{/foreach}
					</table>
					{if $submission->getRecommendation() === null || $submission->getRecommendation() === ''}
						<form role="form" method="post" action="{url op="uploadReviewerVersion"}" enctype="multipart/form-data">
							<input type="hidden" name="reviewId" value="{$reviewId|escape}" />
							<div class="form-group"><input type="file" name="upload" {if not $confirmedStatus or $declined or $submission->getCancelled()}disabled="disabled"{/if} class="form-control" /></div>
					...
					<table class="table table-striped" width="100%">
						<tr valign="top">
							...
							<td class="value" width="70%">
								...
								{else}
									<form role="form" id="recommendation" method="post" action="{url op="recordRecommendation"}">
										<input type="hidden" name="reviewId" value="{$reviewId|escape}" />
										<div class="form-group">
											<select name="recommendation" {if not $confirmedStatus or $declined or $submission->getCancelled() or (!$reviewFormResponseExists and !$reviewAssignment->getMostRecentPeerReviewComment() and !$uploadedFileExists)}disabled="disabled"{/if} class="form-control">
...
{if $journal->getLocalizedSetting('reviewGuidelines') != ''}
	...
	<div id="reviewerGuidelines" class="col-md-12 mag-innert-left">
		<h3>{translate key="reviewer.article.reviewerGuidelines"}</h3>
		<p class="help-block">{$journal->getLocalizedSetting('reviewGuidelines')|nl2br}</p>
	</div>
{/if}

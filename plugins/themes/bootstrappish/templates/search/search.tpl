...
<div id="search">
	...
	<form role="form" id="searchForm" action="{url op="search"}">
		<div class="table-responsive">
			<table class="table table-striped">
				<tr valign="top">
					<td><label class="control-label" for="query">{translate key="search.searchAllCategories"}</label></td>
					<td class="value">
						...
						{if empty($queryFilter)}
							<div class="form-group"><input type="text" id="query" name="query" size="40" maxlength="255" value="{$query|escape}" class="form-control" /></div>
						{else}
							{$queryFilter}
						{/if}
				...
				{if $siteSearch}
					<tr valign="top">
						<td><label class="control-label" for="searchJournal">{translate key="search.withinJournal"}</label></td>
						<td class="value"><div class="form-group"><select name="searchJournal" id="searchJournal" class="form-control">{html_options options=$journalOptions selected=$searchJournal}</select></div></td>
					</tr>
				{/if}
				{if $hasActiveFilters}
					<tr valign="top">
		...
		<button class="btn btn-block btn-info" type="button" data-toggle="collapse" href="#hide-me"><span class="glyphicon glyphicon-th"></span></button>
		{if $hasEmptyFilters}
			<div id="hide-me" class="collapse">
				...
				<p><input type="submit" value="{translate key="common.search"}" class="btn btn-info" /></p>	
			</div>
		{/if}
	</form>
</div>
...
<div id="results">
	<div class="table-responsive">
		<table width="100%" class="table table-striped">
			...
			<tr class="heading" valign="bottom">
				{if !$currentJournal}<td width="20%"><label>{translate key="journal.journal"}</label></td>{/if}
				<td width="{if !$currentJournal}20%{else}40%{/if}"><label>{translate key="issue.issue"}</label></td>
				<td width="60%" colspan="2"><label>{translate key="article.title"}</label></td>
			</tr>
			<tr><td colspan="{$numCols|escape}" class="headseparator">&nbsp;</td></tr>
	...
	{if empty($syntaxInstructions)}
		<ul class="stay">{translate key="search.syntaxInstructions"}</ul>
	{else}
		{* Must be properly escaped in the controller as we potentially get HTML here! *}
		<ul class="stay">{$syntaxInstructions}</ul>
	{/if}
</div>

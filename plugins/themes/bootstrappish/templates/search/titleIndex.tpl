...
<div id="results" class="table-responsive">
	<table width="100%" class="table table-striped">
		...
		<tr class="heading" valign="bottom">
			...
			<td width="20%"><label>{translate key="issue.issue"}</label></td>
			<td width="{if !$currentJournal}60%{else}80%{/if}" colspan="2"><label>{translate key="article.title"}</label></td>
		</tr>
		<tr><td colspan="{$numCols|escape}" class="headseparator">&nbsp;</td></tr>
		...
		{if $results->wasEmpty()}
		<tr>
			<td colspan="{$numCols|escape}" class="nodata"><p class="help-block">{translate key="search.noResults"}</p></td>
		</tr>

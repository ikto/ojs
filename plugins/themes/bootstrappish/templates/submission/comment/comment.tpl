...
{if not $isLocked}
	<div id="postComment" class="col-md-12 mag-innert-left">
		<form role="form" method="post" action="{url op=$commentAction}">
			...
			<div id="new">
				...
				<p><input type="submit" name="save" value="{translate key="common.save"}" class="btn btn-success" /> <input type="submit" name="saveAndEmail" value="{translate key="common.saveAndEmail"}" class="btn btn-success" /> <input type="button" value="{translate key="common.close"}" class="btn btn-danger" onclick="window.close()" /></p>

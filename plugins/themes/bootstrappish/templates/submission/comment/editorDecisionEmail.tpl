...
<tr valign="top">
	...
	<td class="value">
		...
		{if $senderEmail}
			<br/>
			<input type="checkbox" name="bccSender" id="bccSender" value="1"{if $bccSender} checked{/if} />&nbsp;&nbsp;<label class="control-label" for="bccSender">{translate key="email.bccSender" address=$senderEmail|escape}</label>
		{/if}
	</td>
</tr>
{/if}{* addressFieldsEnabled *}
...
<tr valign="top">
	...
	<td class="value">
		...
		<input type="file" name="newAttachment" class="uploadField" /> <input name="addAttachment" type="submit" class="btn btn-success" value="{translate key="common.upload"}" />
	</td>
</tr>
{/if}
{if $isAnEditor}
...
{if $isAnEditor}
	<td>&nbsp;</td>
	<td class="value"><input type="checkbox" name="blindCcReviewers" value="1" id="blindCcReviewers"/>&nbsp;&nbsp;<label class="control-label" for="blindCcReviewers">{translate key="submission.comments.blindCcReviewers"}</label></td>
{/if}
</table>
</div>

<p><input name="send" type="submit" value="{translate key="email.send"}" class="btn btn-success" /> <input type="button" value="{translate key="common.cancel"}" class="btn btn-danger" onclick="history.go(-1)" />{if !$disableSkipButton} <input name="send[skip]" type="submit" value="{translate key="email.skip"}" class="button" />{/if}</p>
</form>
</div>

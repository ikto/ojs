...
<div id="myJournals" class="col-md-12 mag-innert-left">
	{if !$currentJournal}<h3>{translate key="user.myJournals"}</h3>{/if}

	{foreach from=$userJournals item=journal}
		<div id="journal-{$journal->getPath()|escape}" class="col-md-12 mag-innert-left">
			{assign var="hasRole" value=1}
					...
					{if $isValid.Editor.$journalId}
						<tr>
							...
							<td>{if $editorSubmissionsCount[0]}
								...
								{else}<span class="help-block">0 {translate key="common.queue.short.submissionsUnassigned"}</span>{/if}
							</td>
							<td>{if $editorSubmissionsCount[1]}
								...
								{else}<span class="help-block">0 {translate key="common.queue.short.submissionsInReview"}</span>{/if}
							</td>
							<td>{if $editorSubmissionsCount[2]}
								...
								{else}<span class="help-block">0 {translate key="common.queue.short.submissionsInEditing"}</span>{/if}
							</td>
					...
					{if $isValid.Author.$journalId}
						...
						<tr>
							...
							<td>{if $authorSubmissionsCount[0]}
								...
								{else}<span class="help-block">0 {translate key="common.queue.short.active"}</span>{/if}
							</td>
...
{if !$hasRole}
	{if $currentJournal}
		<div id="noRolesForJournal" class="col-md-12 mag-innert-left">
			<p class="text">{translate key="user.noRoles.noRolesForJournal"}</p>
			<ul class="stay">
				<li>
					{if $allowRegAuthor}
	...
	{else}{* $currentJournal *}
		<div id="currentJournal">
			<p class="text">{translate key="user.noRoles.chooseJournal"}</p>
			<ul class="stay">
...
<div id="myAccount" class="col-md-12 mag-innert-left">

...
<div class="col-md-12 mag-innert-left">
	...
	<p class="help-block">{translate key="user.subscriptions.statusInformation"}</p>
	<div class="table-responsive">
		<table width="100%" class="table table-striped">
			...
			<tr class="heading" valign="bottom">
				<td width="30%"><label>{translate key="user.subscriptions.status"}</label></td>
				<td width="70%"><label>{translate key="user.subscriptions.statusDescription"}</label></td>
			</tr>
			<tr>
				<td colspan="2" class="headseparator">&nbsp;</td>
			</tr>
	...
	{if $individualSubscriptionTypesExist}
		...
		<p class="help-block">{translate key="subscriptions.individualDescription"}</p>
		<div class="table-responsive">
			<table width="100%" class="table table-striped">
				{if $userIndividualSubscription}
					<tr valign="top">
						...
						{if $journalPaymentsEnabled && $acceptSubscriptionPayments && $subscriptionStatus == $smarty.const.SUBSCRIPTION_STATUS_AWAITING_ONLINE_PAYMENT}
							<span class="help-block">{translate key="subscriptions.status.awaitingOnlinePayment"}</span>	
						{elseif $journalPaymentsEnabled && $acceptSubscriptionPayments && $subscriptionStatus == $smarty.const.SUBSCRIPTION_STATUS_AWAITING_MANUAL_PAYMENT}
							<span class="help-block">{translate key="subscriptions.status.awaitingManualPayment"}</span>	
						{elseif $subscriptionStatus != $smarty.const.SUBSCRIPTION_STATUS_ACTIVE}
							<span class="help-block">{translate key="subscriptions.inactive"}</span>	
						{else}
							{if $isNonExpiring}
								{translate key="subscriptionTypes.nonExpiring"}
							{else}
								{assign var="isExpired" value=$userIndividualSubscription->isExpired()}
								{if $isExpired}<span class="help-block">{translate key="user.subscriptions.expired"}: {$userIndividualSubscription->getDateEnd()|date_format:$dateFormatShort}</span>{else}{translate key="user.subscriptions.expires"}: {$userIndividualSubscription->getDateEnd()|date_format:$dateFormatShort}{/if}
							{/if}
						{/if}
						</td>
	...
	{if $institutionalSubscriptionTypesExist}
		...
		<p class="help-block">{translate key="subscriptions.institutionalDescription"}{if $journalPaymentsEnabled && $acceptSubscriptionPayments} {translate key="subscriptions.institutionalOnlinePaymentDescription"}{/if}</p>
		<div class="table-responsive">
			<table width="100%" class="table table-striped">
				{if $userInstitutionalSubscriptions}
					...
					<tr valign="top">
						...
						{elseif $journalPaymentsEnabled && $acceptSubscriptionPayments && $subscriptionStatus == $smarty.const.SUBSCRIPTION_STATUS_NEEDS_APPROVAL}
							<span class="help-block">{translate key="subscriptions.status.needsApproval"}</span>	
						{elseif $subscriptionStatus != $smarty.const.SUBSCRIPTION_STATUS_ACTIVE}
							<span class="help-block">{translate key="subscriptions.inactive"}</span>	
						{else}	
							{if $isNonExpiring}
							...
							{else}
								...
								{if $isExpired}<span class="help-block">{translate key="user.subscriptions.expired"}: {$userInstitutionalSubscription->getDateEnd()|date_format:$dateFormatShort}</span>{else}{translate key="user.subscriptions.expires"}: {$userInstitutionalSubscription->getDateEnd()|date_format:$dateFormatShort}{/if}
							{/if}
						{/if}
						</td>
						<td width="20%" align="right">

...
<div class="col-md-12 mag-innert-left"><p class="help-block">{translate key="manager.languages.languageInstructions"}</p></div>
						...
						<table class="table table-striped" width="100%">
							...
							{foreach from=$availableLocales key=localeKey item=localeName}
								<tr>
									...
									<td align="center"><div class="form-group"><input type="checkbox" name="supportedLocales[]" value="{$localeKey|escape}"{if in_array($localeKey, $supportedLocales)} checked="checked"{/if}/></div></td>
									<td align="center"><div class="form-group"><input type="checkbox" name="supportedSubmissionLocales[]" value="{$localeKey|escape}"{if in_array($localeKey, $supportedSubmissionLocales)} checked="checked"{/if}/></div></td>
									<td align="center"><div class="form-group"><input type="checkbox" name="supportedFormLocales[]" value="{$localeKey|escape}"{if in_array($localeKey, $supportedFormLocales)} checked="checked"{/if}/></div></td>
...
{else}
	...
	<p class="help-block">{translate key="manager.languages.noneAvailable"}</p>
{/if}

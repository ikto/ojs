...
<div id="selectMergeUsers" class="col-md-12 mag-innert-left">
	<p class="help-block">{if !empty($oldUserIds)}{translate key="admin.mergeUsers.into.description"}{else}{translate key="admin.mergeUsers.from.description"}{/if}</p>
	<div id="roles" class="col-md-12 mag-innert-left">
		...
		<form role="form" class="form-inline" method="post" action="{url path=$roleSymbolic oldUserIds=$oldUserIds}">
			<div class="form-group">
				<select class="form-control" name="roleSymbolic">
	...
	<div id="users">
		...
		{/if}
				...
				<table width="100%" class="table table-striped">
					...
					<tr class="heading" valign="bottom">
						{if empty($oldUserIds)}
							<td width="5%">&nbsp;</td>
						{/if}
						<td><label>{translate key="user.username"}</label></td>
						<td width="29%"><label>{translate key="user.name"}</label></td>
						<td width="29%"><label>{translate key="user.email"}</label></td>
					...
					{if $users->wasEmpty()}
						<tr>
							<td colspan="{$numCols}" class="nodata"><p class="help-block">{translate key="admin.mergeUsers.noneEnrolled"}</p></td>
						</tr>
						<tr>
			...
			{if empty($oldUserIds)}
				<input type="submit" class="btn btn-warning" value="{translate key="admin.mergeUsers"}" />
		</form>
			{/if}
	</div>
</div>

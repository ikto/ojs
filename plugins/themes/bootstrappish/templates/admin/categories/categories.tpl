...
<div class="col-md-12 mag-innert-left">
	<form role="form" action="{url op="setCategoriesEnabled"}" method="post">
		<p class="help-block">{translate key="admin.categories.enable.description"}</p>
		<div class="form-group">
			<input type="radio" id="categoriesEnabledOff" {if !$categoriesEnabled}checked="checked" {/if}name="categoriesEnabled" value="0"/>&nbsp;<label class="control-label" for="categoriesEnabledOff">{translate key="admin.categories.disableCategories"}</label><br/>
		</div>
		<div class="form-group">
			<input type="radio" id="categoriesEnabledOn" {if $categoriesEnabled}checked="checked" {/if}name="categoriesEnabled" value="1"/>&nbsp;<label class="control-label" for="categoriesEnabledOn">{translate key="admin.categories.enableCategories"}</label><br/>
		</div>
...
<div id="categories" class="table-responsive">
	<table width="100%" class="table table-striped" id="dragTable">
		...
		<tr class="heading" valign="bottom">
			<td width="75%"><label>{translate key="admin.categories.name"}<label></td>
		...
		{if $categories->wasEmpty()}
			<tr>
				<td colspan="2" class="nodata"><p class="help-block">{translate key="admin.categories.noneCreated"}</p></td>
			</tr>
			<tr>

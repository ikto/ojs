...
<div id="copyedit" class="col-md-12 mag-innert-left">
	...
	<div class="table-responsive">
		<table width="100%" class="table table-striped">
			<tr>
				...
				<td width="18%" class="heading"><label>{translate key="submission.underway"}</label></td>
				<td width="18%" class="heading"><label>{translate key="submission.complete"}</label></td>
				<td width="18%" class="heading"><label>{translate key="submission.acknowledge"}</label></td>
			</tr>
			<tr>
				<td width="2%">1.</td>
	...
	{/if}
	<form role="form" method="post" action="{url op="uploadCopyeditVersion"}"  enctype="multipart/form-data">
		<input type="hidden" name="articleId" value="{$submission->getId()}" />
		{translate key="submission.uploadFileTo"}
		<div class="form-group">
		<input type="radio" name="copyeditStage" id="copyeditStageInitial" value="initial" checked="checked" /><label class="control-label" for="copyeditStageInitial">{translate key="navigation.stepNumber" step=1}</label>,
		<input type="radio" name="copyeditStage" id="copyeditStageAuthor" value="author"{if $canUploadCopyedit == 1} disabled="disabled"{else} checked="checked"{/if} /><label class="control-label" for="copyeditStageAuthor"{if $canUploadCopyedit == 1} class="disabled"{/if}>{translate key="navigation.stepNumber" step=2}</label>, {translate key="common.or"}
		<input type="radio" name="copyeditStage" id="copyeditStageFinal" value="final"{if $canUploadCopyedit != 3} disabled="disabled"{else} checked="checked"{/if} /><label class="control-label" for="copyeditStageFinal"{if $canUploadCopyedit != 3} class="disabled"{/if}>{translate key="navigation.stepNumber" step=3}</label>
		<input type="file" name="upload" size="10" class="form-control"{if !$canUploadCopyedit} disabled="disabled"{/if} />
		</div>
		<input type="submit" value="{translate key="common.upload"}" class="btn btn-success"{if !$canUploadCopyedit} disabled="disabled"{/if} />
	</form>

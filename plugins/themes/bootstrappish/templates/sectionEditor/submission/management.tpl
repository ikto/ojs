	...
	<div class="table-responsive">
		<table width="100%" class="table table-striped">
			...
			<tr>
				...
				<td class="value"><form role="form" action="{url op="updateSection" path=$submission->getId()}" method="post">{translate key="submission.changeSection"} <div class="form-group"><select name="section" size="1" class="form-control">{html_options options=$sections selected=$submission->getSectionId()}</select></div> <input type="submit" value="{translate key="common.record"}" class="btn btn-success" /></form></td>
			</tr>

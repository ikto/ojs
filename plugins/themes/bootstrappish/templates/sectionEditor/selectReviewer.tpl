...
<div id="selectReviewer" class="col-md-12 mag-innert-left">
	<h3>{translate key="editor.article.selectReviewer"}</h3>
	
	<form role="form" class="form-inline" id="submit" method="post" action="{url op="selectReviewer" path=$articleId}">
	...
	<div id="reviewers">
		<div class="table-responsive">
			<table class="table table-striped" width="100%">
				...
				<tr class="heading" valign="bottom">
					<td width="20%"><label>{sort_search key="user.name" sort="reviewerName"}</label></td>
					<td><label>{translate key="user.interests"}</label></td>
					{if $rateReviewerOnQuality}
						<td width="7%"><label>{sort_search key="reviewer.averageQuality" sort="quality"}</label></td>
					{/if}
					<td width="7%"><label>{sort_search key="reviewer.completedReviews" sort="done"}</label></td>
					<td width="7%"><label>{sort_search key="editor.submissions.averageTime" sort="average"}</label></td>
					<td width="13%"><label>{sort_search key="editor.submissions.lastAssigned" sort="latest"}</label></td>
					<td width="5%"><label>{sort_search key="common.active" sort="active"}</label></td>
					<td width="7%" class="heading"><label>{translate key="common.action"}</label></td>
				</tr>
				<tr><td colspan="{$numCols|escape}" class="headseparator">&nbsp;</td></tr>
				...
				{if $reviewers->wasEmpty()}
					<tr>
						<td colspan="{$numCols|escape}" class="nodata"><p class="help-block">{translate key="manager.people.noneEnrolled"}</p></td>
					</tr>
		...
		<p class="help-block">{translate key="editor.article.selectReviewerNotes"}</p>
	</div>
</div>
